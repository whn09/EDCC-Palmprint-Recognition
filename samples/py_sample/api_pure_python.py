...
from show_palmprint import build_filters, get_contours, enhance_image, get_encoding, ExecuteMatchingWhenFastCodingMode
...
def encode():
    ...
    if file:
        ...
        start = time.time()
        outfilename = enhance_image(filename)
        end = time.time()
        cherrypy.log('enhance_image time:' + str(end-start))
        encoding = get_encoding(outfilename, filters)
        encoding_str = ''
        for code in encoding:
            encoding_str += chr(code)
        encoding = encoding_str
        #cherrypy.log('encoding:' + encoding)
        #cherrypy.log('codingLen:' + str(codingLen))
        end2 = time.time()
        cherrypy.log('get_encoding time:' + str(end2 - end))
...
def compare():
    ...
    if file1 and file2:
        ...
        outfilename1 = enhance_image(filename1)
        outfilename2 = enhance_image(filename2)
        encoding1 = get_encoding(outfilename1, filters)
        encoding2 = get_encoding(outfilename2, filters)
        start = time.time()
        similarity = ExecuteMatchingWhenFastCodingMode(encoding1, encoding2)
        end = time.time()
        cherrypy.log('ExecuteMatchingWhenFastCodingMode time:' + str(end - start))
...
def search():
    ...
    if file:
        filename = str(os.path.join('upload/', file.filename.split('/')[-1]))
        file.save(filename)
        encoding = get_encoding(filename, filters)
        ...
        for trainPalmprintCode in sample.palmprintcodelist:
            similarity = ExecuteMatchingWhenFastCodingMode(encoding, trainPalmprintCode.code)
